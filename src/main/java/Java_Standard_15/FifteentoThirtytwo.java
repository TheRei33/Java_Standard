...
public class FifteentoThirtytwo {
    public static void main(String[] args) {
        if(args.length != 1 || args[0].length() != 1 || "tTlLnN".indexOf(args[0]) == -1) {
            System.out.println("USAGE : java jsp15_32 SORT_OPTION");
        ...
        for(int i=0; i < files.length; i++) {
            ...
            System.out.printf("%s %3s %6s %s%n", df.format(new Date(f.lastModified())), attribute, name);
        }
    }
}
